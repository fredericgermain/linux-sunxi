...
static int sun4i_usb_phy_init(struct phy *_phy)
{
	...
	if (phy->index == 0) {
		...
		if (data->id_det_gpio) {
			/* OTG mode */
			sun4i_usb_phy0_set_id_detect(_phy, 1);
			sun4i_usb_phy0_set_vbus_detect(_phy, 0);
			data->id_det = 1;
			data->vbus_det = 0;
			/* Scan current status */
...
static int sun4i_usb_phy_power_on(struct phy *_phy)
{
	...
	/* Fpr phy0 only turn on Vbus if we don't have an ext. Vbus */
...
/* FIXME REMOVE ME */
void sunxi_musb_id_change(bool new_id);
...
static void sun4i_usb_phy0_id_vbus_det_scan(struct work_struct *work)
{
	...
	int id_det, vbus_det, id_notify = 0, vbus_notify = 0, debounce = 0;
	...
	if (id_det != data->id_det) {
		...
		data->id_det = id_det;
		/*
		 * We expect a Vbus change soon after an id change, and
		 * we must report this within OTG_TIME_A_WAIT_VRISE msec.
		 */
		debounce = 1;
		id_notify = 1;
	}
	...
	/* FIXME use extcon */
	if (id_notify)
		sunxi_musb_id_change(data->id_det);

	if (data->phy0_poll)
		queue_delayed_work(system_wq, &data->detect,
				   debounce ? DEBOUNCE_TIME : POLL_TIME);
}
...
static irqreturn_t sun4i_usb_phy0_id_vbus_det_irq(int irq, void *dev_id)
{
	...
	queue_delayed_work(system_wq, &data->detect, DEBOUNCE_TIME);
